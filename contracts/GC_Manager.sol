...
contract GC_Manager {
    ...
    function verify() private view {
        if (msg.sender != manager && !admins[msg.sender]) revert("Unauthorised Action!");
    }
    ...
    verify();
    require(Championships[year] != address(0), "Championship doesn't exist or is deactivated!");
    require(GC(Championships[year]).is_active(), "Championship doesn't exist or is deactivated!");
