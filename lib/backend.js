...
GC_Manager.on("GC_Created", async (AY, address) => {
    ...
    GC_Ctrct.on("Hostels_event", async (ev,participant,hostel)=>{
        hostel=ethers.decodeBytes32String(hostel);
        if(ev==="added"){
            db.participant.add(participant,hostel,AY)
            db.participant.verify(participant,hostel)
        }
        if(ev==="removed"){
            db.participant.remove(participant,hostel,AY)
        }
    })
...
export async function GET(request) {
    ...
    switch (searchParams.get("task")) {
        ...
        case "Get_Status":
            ...
            break
        case "Get_Hostels":
            const participants = db.partcipant.get()
        ...
        case "RPC_URL":
            ...
            return NextResponse.json({ RPC_URL: data.RPC_URL });
        break;
        case "Get_ABI":
            if (!(await GC_Manager.verify_caller(address))) NextResponse.json({ status: "Unauthorised Action" }, { status: 401 })
